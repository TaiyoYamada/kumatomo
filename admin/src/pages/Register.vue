<template>
  <v-container class="fill-height d-flex align-center justify-center">
    <v-card class="pa-6" max-width="420" elevation="2">
      <v-card-title class="text-h5 font-weight-bold pb-2">新規登録</v-card-title>
      ...
      <v-form ref="formRef" v-model="valid" @submit.prevent="onSubmit">
        ...
        <v-text-field v-model="password" label="パスワード" type="password" variant="outlined"
          :rules="passwordRules" density="comfortable" class="mb-3" required></v-text-field>

        <v-text-field v-model="passwordConfirm" label="パスワード（確認）" type="password" variant="outlined"
          :rules="confirmRules" density="comfortable" required></v-text-field>

        <v-card-actions class="px-0 pt-6">
          <v-btn :loading="loading" type="submit" color="primary" block>登録</v-btn>
        </v-card-actions>
      </v-form>
...
const onSubmit = async () => {
  ...
  try {
    ...
    error.value = e?.response?.data?.message || e?.message || '新規登録に失敗しました'
  } finally {
    loading.value = false
  }
}
