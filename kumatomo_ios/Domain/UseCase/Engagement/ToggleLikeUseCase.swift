...
protocol ToggleLikeUseCase {
    func execute(postId: Int, currentState: Bool, currentCount: Int) async -> Result<(isLiked: Bool, likeCount: Int), EngagementError>
}
...
final class ToggleLikeUseCaseImpl: ToggleLikeUseCase {
    ...
    func execute(postId: Int, currentState: Bool, currentCount: Int) async -> Result<(isLiked: Bool, likeCount: Int), EngagementError> {
        let result = await repository.optimisticToggleLike(postId: postId, currentState: currentState, currentCount: currentCount)
