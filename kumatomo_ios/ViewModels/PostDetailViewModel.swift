...
class PostDetailViewModel: ObservableObject {
    ...
    private let commentAPIService = CommentAPIService.shared
    ...
    func loadPostDetail(postId: Int) async {
        ...
        do {
            let fetchedPost = try await postAPIService.fetchPost(postId: postId)
            post = fetchedPost
    ...
    func loadComments(postId: Int) async {
        ...
        do {
            let fetchedComments = try await commentAPIService.fetchComments(postId: postId)
    ...
    func addComment(_ text: String, image: UIImage? = nil) async {
        ...
        do {
            let newComment = try await commentAPIService.createComment(
                postId: currentPost.id,
                content: trimmedText,
                image: image
            )
