...
- 熊本県限定：投稿・閲覧は熊本県のユーザーに限定され、地域ごとの話題で盛り上がれるローカルSNS。

- 市区町村別掲示板：熊本市・宇城市など市ごとに分かれた掲示板構成。地元の情報をピンポイントで収集・共有。

- 簡単な投稿と認証：メール + パスワード + 認証コードでのシンプルなサインアップ。

- 画像投稿対応：画像付きの投稿も可能（外部サービス ImgBB 経由で保存予定）。

- 管理者用ダッシュボード：Vue.js + Vuetify製のSPAで通報や投稿の管理が可能。
